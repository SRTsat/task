...
interface PaymentMethod {
  ...
  enabled: boolean;
  configured?: boolean; // true when frontend client key / config available
}
...
export function PaymentProcessor({ 
  ...
  useEffect(() => {
    // quick runtime visibility in console to debug why gateways don't show
    console.debug('PaymentProcessor: paymentMethods prop =', paymentMethods);
    console.debug('PaymentProcessor: plan.paymentMethods =', plan.paymentMethods);
  }, [paymentMethods, plan.paymentMethods]);
  
  // Safe fallback: if paymentMethods prop is provided and non-empty use it,
  // otherwise try to derive a displayable list from plan.paymentMethods (often an object).
  const enabledPaymentMethods: PaymentMethod[] = (() => {
    const pmList = Array.isArray(paymentMethods) ? paymentMethods : [];
    if (pmList.length > 0) {
      // if incoming prop list doesn't include configured flag, assume true
      return pmList.filter(m => !!m && !!m.enabled).map(m => ({ ...m, configured: m.configured ?? true }));
    }

    // normalize plan.paymentMethods: some backends nest settings under payment_settings or paymentSettings
    const rawPlanPM = plan.paymentMethods || {};
    const planPM = rawPlanPM.payment_settings || rawPlanPM.paymentSettings || rawPlanPM;
    if (!planPM || typeof planPM !== 'object' || Object.keys(planPM).length === 0) {
      return [];
    }

    // Map of known keys to user-friendly names (extend as needed)
    ...
    const knownKeys = Object.keys(nameMap);
    const methods: PaymentMethod[] = [];

    knownKeys.forEach((key) => {
      const cfg = (planPM as any)[key];
      // Check multiple patterns:
      const enabledByObject = cfg && typeof cfg === 'object' && (cfg.enabled === true || cfg.status === 'enabled' || cfg.client_key || cfg.public_key || cfg.api_key);
      const enabledByFlag = (planPM as any)[`${key}_enabled`] === true || (planPM as any)[`is_${key}_enabled`] === true;
      const enabledByStatus = (planPM as any)[`${key}_status`] === 'enabled' || (planPM as any)[`${key}_status`] === 'active';
      const enabledByPresence = !!cfg && cfg === true;
      const enabledByMidtransKeys = key === 'midtrans' && (!!(planPM as any)['midtrans_client_key'] || !!(planPM as any)['midtrans_secret_key']);
      const isEnabled = !!(enabledByObject || enabledByFlag || enabledByStatus || enabledByPresence || enabledByMidtransKeys);

      if (isEnabled) {
        // Determine if frontend config (client key) exists — especially important for Midtrans
        let configured = true;
        if (key === 'midtrans') {
          // check both top-level and nested payment_settings sources
          configured = !!(
            (planPM as any)['midtrans_client_key']
            || (planPM as any)['midtransClientKey']
            || (planPM as any)['midtrans_public_key']
            || (rawPlanPM as any)['midtrans_client_key']
            || (rawPlanPM as any)['payment_settings']?.['midtrans_client_key']
            || (rawPlanPM as any)['paymentSettings']?.['midtrans_client_key']
          );
        } else {
          // for generic method, try to detect a public/client key if config object exists
          if (cfg && typeof cfg === 'object') {
            configured = !!(cfg.client_key || cfg.public_key || cfg.api_key || cfg.access_token);
          }
        }
        methods.push({
          id: key,
      ...
      case 'midtrans':
        return (
          <MidtransPaymentForm
            {...commonProps}
            planPrice={Number(plan.price)}
            // prefer client key; fallback to other possible names (do NOT send secret to client if absent)
            midtransClientKey={
              plan.paymentMethods?.midtrans_client_key
              || plan.paymentMethods?.midtransClientKey
              || plan.paymentMethods?.midtrans_key
              || plan.paymentMethods?.midtrans_public_key
              || ''
            }
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        {enabledPaymentMethods.length === 0 ? (
          <>
            <p className="text-sm text-muted-foreground">
              {t('No payment methods are currently available. Please contact support for assistance.')}
            </p>
            <div className="flex items-center gap-2">
              <Button variant="ghost" size="sm" onClick={() => setShowDebug(v => !v)}>
                {showDebug ? t('Hide debug info') : t('Show debug info')}
              </Button>
            </div>
            {showDebug && (
              <pre className="mt-2 p-2 bg-gray-50 text-xs rounded max-h-48 overflow-auto">
                {JSON.stringify({ paymentMethodsProp: paymentMethods, planPaymentMethods: plan.paymentMethods }, null, 2)}
              </pre>
            )}
          </>
        ) : (
         <div className="space-y-2">
           {enabledPaymentMethods.map((method, index) => (
              <Card 
                ...
                onClick={() => {
                  // prevent selecting if not configured
                  if (method.enabled && method.configured !== false) {
                    setSelectedPaymentMethod(method.id);
                  } else {
                    toast.error(t('This payment method is not configured. Please contact the administrator.'));
                  }
                }}
              >
                <CardContent className="p-3">
                  <div className="flex items-center gap-3">
                    ...
                    {method.configured === false && (
                      <Badge variant="outline" className="ml-auto text-yellow-700">
                        {t('Not configured')}
                      </Badge>
                    )}
                    {selectedPaymentMethod === method.id && (
