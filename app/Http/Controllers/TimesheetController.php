...
class TimesheetController extends Controller
{
    ...
    private function getAccessibleProjects($user, $workspace)
    {
        ...
        return Project::where('workspace_id', $workspace->id)
            ...
            ->with(['tasks' => function($query) use ($user) {
                $query->with(['collaborators.user'])
                    ->whereHas('collaborators', function($q) use ($user) {
                        $q->where('user_id', $user->id);
                    })
                    ->orWhere(function($taskQuery) use ($user) {
                        $taskQuery
                            ->whereHas('taskStage', function($q) use ($user) {
                                $q->whereNotIn('name', ['Done', 'On Hold']);
                            })
    ...
    public function store(Request $request)
    {
        ...
        // validate start_date only one per day
        if ($user->timesheets()->where('workspace_id', $workspace->id)->where(function($q) use ($validated) {
                $q->where('start_date', $validated['start_date']);
            })->exists())
        {
            return back()->withErrors(['start_date' => 'You already have a timesheet for the selected date']);
        }
    ...
    public function dailyView(Request $request)
    {
        ...
        $query = TimesheetEntry::with(['project', 'task', 'timesheet', 'user'])
        ...
        // Apply assignee filter (only for owner/admin/manager)
        if ($request->user_id && $request->user_id !== 'all') {
            $query->where('user_id', $request->user_id);
        }

        // Apply done status filter
        if ($request->done && $request->done !== 'all') {
            if ($request->done === 'true') {
                $query->whereHas('timesheet', function($q) {
                    $q->where('is_done', true);
                });
            } elseif ($request->done === 'false') {
                $query->whereHas('timesheet', function($q) {
