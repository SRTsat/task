...
class PaymentSettingController extends Controller
{
    ...
    public function getPaymentMethods()
    {
        ...
        // // Ensure midtrans client key is exposed to frontend (if present)
        // $paymentSettings['midtrans_client_key'] = $paymentSettings['midtrans_client_key'] ?? ($settings['midtrans_client_key'] ?? null);
    ...
    public function store(Request $request)
    {
        ...
        try {
            $validatedData = $request->validate([
                ...
                'midtrans_mode' => ['required', 'in:sandbox,live'],
                'midtrans_secret_key' => [
                    'nullable',
                    'string',
                    function ($attribute, $value, $fail) {
                        if (empty($value) && empty(env('MIDTRANS_SERVER_KEY'))) {
                            $fail(__('The Midtrans secret key is missing (not in config or .env).'));
                        }
                    }
                ],
                'midtrans_client_key' => [
                    ...
                    'string',
                    function ($attribute, $value, $fail) {
                        if (empty($value) && empty(env('MIDTRANS_CLIENT_KEY'))) {
                            $fail(__('The Midtrans client key is missing (not in config or .env).'));
                        }
                    }
                ],
    ...
    private function preparePaymentSettings(Request $request, array $validatedData): array
    {
        return [
            ...
            'midtrans_client_key' => $validatedData['midtrans_client_key'],
    ...
    private function validateEnabledPaymentMethods(Request $request, array $validatedData): void
    {
        ...
        if ($request->boolean('is_midtrans_enabled')) {
            $validation = validatePaymentMethodConfig('midtrans', []);
            if (!$validation['valid']) {
                $errors = array_merge($errors, $validation['errors']);
            }
        }
