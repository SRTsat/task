...
class TimesheetEntryController extends Controller
{
    ...
    public function update(Request $request, TimesheetEntry $timesheetEntry)
    {
        try {
            ...
            if ($validated['hours'] == null) $validated['hours'] = ($timesheetEntry->end_time && $timesheetEntry->start_time) ? (strtotime($timesheetEntry->end_time) - strtotime($timesheetEntry->start_time)) / 3600 : 0;
            ...
            return redirect()->back()->with('error', __('Failed to update time entry. Please try again. ('.$e->getMessage().')') );
        }
    }
